...
%                                                          1        2    3                     4                                   5                6                                                 
% 这是前面计算像差的参数表，具体的参数见如下
% 表示iteration的次数是50，这里的sCMOSvarmap数值为0
% 像散的输出：X, Y, Photons, Background, Z, Iterations 
...
narginh=nargin;  % 这里narginh = nargin = 6
%determine of it runs on GPU, otherwise use CPU as default
persistent fitter    % 持久变量，在这个函数里面fitter的值会保留(也就是下一次调用的时候就是上一次结束时候的值)
allfitters={@GPUmleFit_LM,@CPUmleFit_LM};  % 这里元胞的数据类型是函数句柄
...
if narginh>3 && ~isempty(varargin{4})
    fitpar=single(varargin{4});  % 这里的fitpar就是coeff， 30*30*197*64
else
