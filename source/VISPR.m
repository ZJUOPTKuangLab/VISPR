% pay attention to modify the initial parameters: NA, lambda, pixelsize...
...
for iter = 1 : iteration
    ...
    for i=1:parameters.sizeZ
        ref_PSF(:,:,i) = ref_PSF(:,:,i).*Nph + bg;
    end
...
%     imageslicer(permute(single(ref_PSF(:,:,index_record_Zplanes==1)),[2 1 3]));
%     title('ref PSF')
% 
%     imageslicer(permute(single(ims_Zcal_ave_plane1_sel),[2 1 3]));
%     title('average PSF')
...
if exist('Zernike','var') == 0
    ...
    plot((P(1:21)./parameters.lambda)*2*pi, 'o-', 'LineWidth', 2,'MarkerSize', 3);
    ...
    % ylim([-70 50]);
    ylabel('Zernike rms value(rad)') ,xlabel('Zernike mode')
else
    aberration_name = ["OAst", "VAst"...
    "Trefoil y","Coma y","Coma x","Trefoil x",...
    "Quadrafoil y","Secondary QAst","Primary sph",...
    "Secondary VAst","quadrafoil x",...
    "Tetrafoil y","Secondary trefoil y","Teritary coma y",...
    "Tetrafoil x","Secondary trefoil x","Teritary coma x",...
    "Hexafoil y","Tertiary Quadrafoil y","Tertiary OAst","Tertiary sph"
    ];
    ...
    range = 1:21;
    plot(range,(aberrations(1:21,3)./parameters_real.lambda)*2*pi, '-o', 'LineWidth', 1.5,'MarkerSize', 8);hold on
    plot(range,(P(1:21)./parameters_real.lambda)*2*pi, '-.diamond', 'LineWidth', 1.5,'MarkerSize', 8);
   
    legend({'Ground truth','VISPR'},'Location','north')
    axn = cell(1,21);
    for k=21:-1:1
        axn{k}=[aberration_name(1,k)];
    end
    xlim([1 21]) 
    axMode.XTick=1:length(axn);
    axMode.XTickLabel=axn;
    xtickangle(45)
    ylabel('Zernike rms value(rad)','FontWeight','bold') ,xlabel('Zernike mode','FontWeight','bold')
